...
export async function loadProgress(studentId: string, now: Date = new Date()): Promise<Progress> {
  const [sessions, attempts] = await Promise.all([
    ...
    Attempt.find({ student_id: studentId })
      ...
      .select('question_id rubric_awarded rubric correct ts')
      .lean<{ _id: unknown; question_id?: OutcomeQuestion | null; rubric_awarded: string[]; rubric?: OutcomeRow[]; correct: boolean; ts: Date }[]>(),
  ]);
