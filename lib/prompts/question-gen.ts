...
- COORDINATES ARE STATED ONCE, IN THE QUESTION, and coordinateGrid has TWO BRANCHES. Pick one; "named" is not how the template is driven.
  · The question STATES coordinates — $A(1,1)$, $C' = (6,1)$: use the "named" branch, which REFERENCES those labels instead of repeating them: {"named": {"polygons": [{"points": ["A","B","C"]}, {"points": ["A'","B'","C'"], "dashed": true}]}}. Leave x_range and y_range out — the window is derived from the question's own values, so the figure cannot disagree with them. Reference ONLY points the question states: if the student's job is to find the image, the image is not in the figure, because drawing it would hand them the answer. It renders as a schematic sketch by default — no axes, no gridlines, no scale numbers, which is what the papers print unless the question is about reading the grid itself; set "sketch": false only when the student must read values off the axes.
  · The question states NO coordinates — draw $y = 2x + 3$, a distance-time graph, a shaded region: OMIT "named" ENTIRELY and use x_range/y_range with lines, curves, points, polygons or regions. An empty "named", or one that names nothing the question states, is not a figure and is rejected.
  · A window is as wide as its mathematics needs, so long as it divides into a readable number of ruled steps — between 2 and 40 of them. A distance-time graph to 120 km is {"y_range": [0, 120], "y_step": 10}; a cost graph to $500 is {"y_range": [0, 500], "y_step": 50}. Give x_step/y_step for any span over 40. Never shrink the mathematics to fit the grid.
...
A CLOZE BLANK NEVER RESTATES A VALUE FOUND EARLIER. "Complete the statement below" is a part like any other: each blank must demand something the candidate has not already been paid for. A blank whose answer is the answer to an earlier part — the value of $x$ found in (b), the total computed in (a) — asks the candidate to copy their own work across, and the mark buys nothing. Put the conclusion in the blank, not the number: "the sets are ___" earns its mark, "and $n(A \\cap B) = $ ___" does not when (b) already asked for that number. If the statement needs the value to read as a sentence, write it into the statement rather than blanking it.
...
FOR EACH PART, ALSO REPORT "new_work". You have just solved the whole question, so you are the only reader who knows what each part actually cost. Set "new_work": true when answering that part required at least one step you had not already done — a calculation, a deduction, a construction, or reading a value off the figure or table. Set it FALSE when the answer was already available: it is stated in the stem or stimulus and only needs repeating, or it is an earlier part's result restated, or it is an earlier part's calculation run backwards so that no new fact is established. A SLOT THAT DRAWS A CONCLUSION, A COMPARISON OR AN INTERPRETATION FROM A VALUE IS NEW WORK, even when the value came from an earlier part and nothing is calculated: deciding that 6 litres does not meet a 7-litre requirement, that a percentage exceeds a threshold, that two sets are therefore not disjoint. The candidate has to know what the value MEANS for the question asked, and that is a step they can get wrong. What is not new work is the value itself appearing again in another blank. "new_work_note" is one short clause saying which: "computed the gradient from two points", or "stated in the stem". Judge what the part DEMANDS, not how it is worded — a part can look like a question and ask for nothing.${
    args.visualText
      ? `
