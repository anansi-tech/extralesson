...
- A PART WITH MORE THAN ONE ANSWER BOX MUST SAY WHAT EACH BOX IS FOR (hard requirement). The label is a key; the wording the student reads is the slot's "prompt". If (b) asks for two things, each slot carries its own prompt naming ITS quantity — "the gradient", "the $y$-intercept", "the coordinates of $Q$" — never one instruction covering both with the boxes left to be told apart by their order. A slot whose LABEL already names the quantity — "centre", "factor", "modal_class" — is doing the same job and needs no prompt; a slot labelled "i" or "ii" is not.
