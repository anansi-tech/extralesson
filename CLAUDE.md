...
`ROUND_13_THE_OPERATOR.md` (the five admin screens, designed rather than
grown: the operator arrives with work or with a name and never to browse,
so each screen is built for those two arrivals — layout from
`design/ui/ExtraLesson Admin *.dc.html`, and no new features), on top of
`ROUND_12_REFUND_AND_REVOKE.md` (the way access ends: refund and revoke in
the safe order, revocation that actually stops a session, the operator's
path and the student's one tap), on top of `ROUND_11_ONE_PAYMENT.md` (one
record for what happened to a payment: the state on `Payment`, one
transactional claim, the queue derived from it, then migration and the
deletion of `Fulfilment`), on top of
...
`ROUND_1_EXTRALESSON.md` — read all eleven before any non-trivial change. The photo-grading examiner engine is Round 2. `feat/question-bank-fingerprint` is
