...
| 8 | 6a51ef | CK3 | Forms the condition $h(x)\ge20$ for retaining at least \$20 | Write the condition for Plan B retaining at least $20$ as $h(x)20$. |
| 9 | 6a51ef | R3 | Solves $3x-6\ge20$ to obtain $x\ge\frac{26}{3}$ | Solve $3x-620$ step by step to find the lower bound for $x$. |
| 10 | 6a51ef | R4 | Justifies that 8 trays give only \$18, which is less than \$20 | Substitute 8 trays into Plan B and show that the retained amount is below $20$. |
| 11 | 6a5256 | CK1 | Writes $\overrightarrow{OA}$ as the column vector $\begin{pmatrix}2\\-1\end{pmatrix}$ | Write the coordinates of A as the components of overrightarrow{OA}. |
| 12 | 6a5256 | CK2 | Writes $\overrightarrow{OB}$ as the column vector $\begin{pmatrix}8\\5\end{pmatrix}$ | Write the coordinates of B as the components of overrightarrow{OB}. |
| 13 | 6a5256 | AK1 | Subtracts position vectors to obtain $\overrightarrow{AB}=\begin{pmatrix}8-2\\5-(-1)\end{pmatrix}$ | Subtract the components of overrightarrow{OA} from the corresponding components of overrightarrow{OB} to find overrightarrow{AB}. |
| 14 | 6a5256 | CK3 | Forms $\overrightarrow{OC}=\overrightarrow{OA}+\dfrac{3}{2}\overrightarrow{AB}$ | Form overrightarrow{OC} by adding overrightarrow{OA} to frac{3}{2}overrightarrow{AB}. |
| 15 | 6a5256 | AK2 | Finds $\dfrac{3}{2}$ of “their” $\overrightarrow{AB}$ | Multiply each component of your earlier overrightarrow{AB} by frac{3}{2}. |
| 16 | 6a5256 | CK4 | Identifies $\overrightarrow{AD}=\overrightarrow{OD}-\overrightarrow{OA}$ and $\overrightarrow{DC}=\overrightarrow{OC}-\overrightarrow{OD}$ | Find overrightarrow{AD} by subtracting overrightarrow{OA} from overrightarrow{OD}, then find overrightarrow{DC} by subtracting overrightarrow{OD} from overrightarrow{OC}. |
| 17 | 6a5256 | R1 | Obtains $\overrightarrow{AD}=\begin{pmatrix}3\\3\end{pmatrix}$ | Subtract the coordinates of A from the corresponding coordinates of D to obtain overrightarrow{AD}. |
| 18 | 6a5256 | R2 | Obtains $\overrightarrow{DC}=\begin{pmatrix}6\\6\end{pmatrix}$ using “their” $\overrightarrow{OC}$ | Subtract the coordinates of D from the corresponding components of your earlier overrightarrow{OC} to obtain overrightarrow{DC}. |
...
| 46 | 6a5278 | AK3 | Solves to obtain $x=\pm2$ | Solve the equation by isolating the squared term and taking both square roots, giving $x=\pm2$. |
...
| 50 | 6a528f | CK1 | Uses $-3$ as the first component of $\overrightarrow{OA}$ | Read station $A$ from the graph and use its horizontal coordinate as the first component of $overrightarrow{OA}$. |
| 51 | 6a528f | CK2 | Uses $2$ as the second component of $\overrightarrow{OA}$ | Read station $A$ from the graph and use its vertical coordinate as the second component of $overrightarrow{OA}$. |
| 52 | 6a528f | CK3 | Uses $2$ as the first component of $\overrightarrow{OB}$ | Read station $B$ from the graph and use its horizontal coordinate as the first component of $overrightarrow{OB}$. |
| 53 | 6a528f | CK4 | Uses $4$ as the second component of $\overrightarrow{OB}$ | Read station $B$ from the graph and use its vertical coordinate as the second component of $overrightarrow{OB}$. |
| 54 | 6a528f | R1 | Forms $\overrightarrow{AB}=\overrightarrow{OB}-\overrightarrow{OA}$ | Form $overrightarrow{AB}=overrightarrow{OB}-overrightarrow{OA}$ to find the displacement from $A$ to $B$. |
...
| 57 | 6a528f | R2 | Forms $\overrightarrow{OC}=\overrightarrow{OB}+2\overrightarrow{AB}$ using "their" $\overrightarrow{AB}$ | Form $overrightarrow{OC}=overrightarrow{OB}+2overrightarrow{AB}$ using your earlier $overrightarrow{AB}$. |
| 58 | 6a528f | AK3 | Finds twice "their" $\overrightarrow{AB}$ | Multiply each component of your earlier $overrightarrow{AB}$ by $2$. |
...
| 62 | 6a52ab | AK2 | Uses $\left(180°-\text{their }\angle PAQ\right)\div2$ to find each base angle | Calculate each base angle using $[0m\left(180°-\text{your }\angle PAQ\right)\div2$. |
...
| 64 | 6a52ab | AK3 | Uses $P$, $O$ and $Q$ collinear to equate $\angle APQ$ and $\angle QPA$ | Use the fact that $P$, $O$ and $Q$ are collinear to set $[0m\angle APQ=\angle QPA$. |
| 65 | 6a52ab | CK4 | Recognises that $TQ\perp OQ$, so $TQ$ is a tangent at $Q$ | Show that $TQ$ is perpendicular to $OQ$, then conclude that $TQ$ is a tangent at $Q$. |
| 66 | 6a52ab | R3 | Uses the alternate-segment angle to obtain $\angle TQA=\text{their }\angle APQ$ | Use the alternate-segment angle to set $[0m\angle TQA=\text{your }\angle APQ$. |
| 67 | 6a52ab | AK4 | Uses angle sum of triangle $ATQ$: $180°-\text{their }\angle TAQ-\text{their }\angle TQA$ | Find $[0m\angle ATQ$ using $180°-\text{your }\angle TAQ-\text{your }\angle TQA$. |
| 68 | 6a52b3 | CK1 | States $\vec{PQ}=\vec{OQ}-\vec{OP}$ | Find the vector from $P$ to $Q$ by subtracting $\vec{OP}$ from $\vec{OQ}$. |
| 69 | 6a52b3 | CK2 | Recognises that each term in $\vec{PQ}$ is multiplied by $2$ | Multiply both the $\vec{a}$ component and the $\vec{b}$ component of $\vec{PQ}$ by $2$ to get $\vec{QR}$. |
| 70 | 6a52b3 | CK3 | States $\vec{OR}=\vec{OQ}+\vec{QR}$ | Travel from $O$ to $R$ through $Q$, so add $\vec{OQ}$ and $\vec{QR}$. |
| 71 | 6a52b3 | AK3 | Adds corresponding components of $\vec{OQ}$ and "their" $\vec{QR}$ | Add the $\vec{a}$ components together and add the $\vec{b}$ components together in $\vec{OQ}$ and your $\vec{QR}$. |
| 72 | 6a52b3 | R2 | Uses "their" $\vec{OR}$ to obtain $\vec{OS}$ and forms $\vec{PS}=\vec{OS}-\vec{OP}$ | Use your $\vec{OR}$ to find $\vec{OS}$, then subtract $\vec{OP}$ from $\vec{OS}$ to form $\vec{PS}$. |
| 73 | 6a52b3 | R3 | Concludes parallelogram since "their" $\vec{PS}$ equals "their" $\vec{QR}$ | Compare your $\vec{PS}$ with your $\vec{QR}$ and conclude that $PQRS$ is a parallelogram when they are equal. |
| 74 | 6a52bb | AK1 | Calculates $6 \times 2 = 12$ | Multiply 6 by 2 and write the product in the first $fx$ entry. |
| 75 | 6a52bb | AK2 | Calculates $7 \times 3 = 21$ | Multiply 7 by 3 and write the product in the second $fx$ entry. |
| 76 | 6a52bb | AK3 | Calculates $8 \times 4 = 32$ | Multiply 8 by 4 and write the product in the third $fx$ entry. |
| 77 | 6a52bb | AK4 | Calculates $9 \times 1 = 9$ | Multiply 9 by 1 and write the product in the fourth $fx$ entry. |
...
| 84 | 6a52c8 | CK2 | Substitutes $V=36\pi$ into the expression for $r$ | Replace \(V\) in your expression for \(r\) with the given tank volume. |
| 85 | 6a52c8 | AK2 | Simplifies $\frac{3(36\pi)}{4\pi}$ to $27$ | Simplify the fraction by cancelling \(\pi\) and carrying out the remaining arithmetic. |
...
| 89 | fe84a7 | CK1 | Uses $\overrightarrow{AB}=\overrightarrow{OB}-\overrightarrow{OA}$ | Use \(\overrightarrow{AB}=\overrightarrow{OB}-\overrightarrow{OA}\) to find the displacement from A to B. |
| 90 | fe84a7 | AK1 | Subtracts components to obtain $\begin{pmatrix}5\\3\end{pmatrix}$ | Subtract the corresponding components of the two position vectors. |
| 91 | fe84a7 | CK2 | Uses $\overrightarrow{BC}=2\times$ "their" $\overrightarrow{AB}$ | Multiply your \(\overrightarrow{AB}\) by \(2\) to form \(\overrightarrow{BC}\). |
| 92 | fe84a7 | AK2 | Calculates $\overrightarrow{BC}=\begin{pmatrix}10\\6\end{pmatrix}$, or equivalent follow-through value | Double each component of your \(\overrightarrow{AB}\). |
| 93 | fe84a7 | CK3 | Uses $\overrightarrow{OC}=\overrightarrow{OB}+\overrightarrow{BC}$ | Add \(\overrightarrow{OB}\) and \(\overrightarrow{BC}\) to find \(\overrightarrow{OC}\). |
| 94 | fe84a7 | AK3 | Adds vectors to obtain $\begin{pmatrix}12\\11\end{pmatrix}$, or correct follow-through from "their" $\overrightarrow{BC}$ | Add the corresponding components of \(\overrightarrow{OB}\) and your \(\overrightarrow{BC}\). |
...
| 98 | fe84a7 | R3 | Finds target position minus "their" $\overrightarrow{OC}$ to obtain $\begin{pmatrix}-2\\0\end{pmatrix}$ | Subtract your \(\overrightarrow{OC}\) from the target position vector to find the required displacement. |
...
| 104 | fe84af | R1 | Compares "their" composite expressions and states $fg\ne gf$ | Compare your two composite expressions and state that they are not equal if they give different expressions. |
...
| 117 | fe84c4 | AK2 | Halves "their" $\angle AOB$. | Divide your $ngle AOB$ by $2$ to find $ngle ACB$. |
...
| 119 | fe84c4 | AK3 | Calculates $180-42-\text{"their" }\angle ACB$. | Subtract $42°$ and your $ngle ACB$ from $180°$ to find $ngle CBA$. |
| 120 | fe84c4 | CK4 | Identifies $\angle CBA$ as the angle in the alternate segment for chord $CA$. | Identify $ngle CBA$ as the angle in the alternate segment for chord $CA$. |
| 121 | fe84c4 | AK4 | States "their" $\angle CBA$ as the required angle between a tangent at $C$ and chord $CA$. | Use your $ngle CBA$ as the angle that a tangent at $C$ must make with chord $CA$. |
...
| 124 | fe84cc | CK1 | Identifies $QR$ as opposite the $30^\circ$ angle | Identify $QR$ as the side opposite the $30^irc$ angle. |
| 125 | fe84cc | CK2 | Selects the tangent ratio | Choose the tangent ratio, using opposite divided by adjacent. |
| 126 | fe84cc | AK1 | Substitutes into $\tan 30^\circ=\frac{QR}{45}$ | Substitute the known side and angle into $	an 30^irc=\frac{QR}{45}$. |
| 127 | fe84cc | AK2 | Evaluates $QR=45\tan30^\circ$ | Calculate $QR$ by evaluating $45\tan30^irc$. |
| 128 | fe84cc | R1 | Gives $26.0\text{ m}$ correct to 1 decimal place | State the mast height in metres, rounded to 1 decimal place. |
...
| 130 | fe84cc | AK3 | Uses $\frac{\text{their }QR}{\sin30^\circ}$ to find $PR$ | Use your value of $QR$ and calculate $PR$ with $\frac{\text{your }QR}{\sin30^irc}$. |
| 131 | fe84cc | AK4 | Evaluates $PR=\frac{25.980\ldots}{\sin30^\circ}=51.961\ldots$ | Evaluate $PR=\frac{25.980\ldots}{\sin30^irc}$. |
| 132 | fe84cc | R2 | Gives $52.0\text{ m}$ correct to 1 decimal place | State the support-cable length in metres, rounded to 1 decimal place. |
| 133 | fe84cc | R3 | Concludes that the cable is not long enough since "their" $PR>51\text{ m}$ | Compare your value of $PR$ with $51\text{ m}$ and conclude whether the cable is long enough. |
...
| 136 | fe84d4 | CK3 | Identifies $P(\text{underweight}) = \frac{30}{100} = 0.3$ | Find the underweight frequency out of the total and write it as a decimal probability. |
| 137 | fe84d4 | R1 | Multiplies probabilities for the independent events using $0.2 \times 0.3$ | Multiply the probability of a damaged bag by the probability of an underweight bag because the events are independent. |
...
| 141 | d9c1e8 | CK1 | Recognises that $\overrightarrow{AB}$ is found by subtracting the coordinates of $A$ from those of $B$. | Subtract the coordinates of $A$ from the corresponding coordinates of $B$ to form $overrightarrow{AB}$. |
| 142 | d9c1e8 | AK1 | Subtracts correctly to obtain $\begin{pmatrix}3\\4\end{pmatrix}$. | Carry out both coordinate subtractions accurately to obtain the displacement vector. |
| 143 | d9c1e8 | CK2 | Uses $\left\|\begin{pmatrix}x\\y\end{pmatrix}\right\|=\sqrt{x^2+y^2}$. | Use $left\|egin{pmatrix}x\\y\end{pmatrix}right\|=sqrt{x^2+y^2}$ to find the vector’s magnitude. |
| 144 | d9c1e8 | AK2 | Evaluates $\sqrt{3^2+4^2}$. | Square the two vector components, add the results, and evaluate the square root accurately. |
...
| 157 | d9c1f0 | R2 | Forms the mass of the order as $200 \times 18 \times 350$ g | Form the order mass in grams using \(200\times18\times350\). |
...
| 160 | d9c1f8 | CK1 | Divides both sides by $2\pi$ to isolate the square root | Divide both sides by $2\pi$ so that the square root is alone. |
...
| 163 | d9c1f8 | AK3 | Substitutes into $l = \frac{gT^2}{4\pi^2}$ using "their" expression from part (a) | Substitute $T = 2.20$ and $g = 9.8$ into your expression from part (a): $l = \frac{gT^2}{4\pi^2}$. |
| 164 | d9c1f8 | AK4 | Evaluates "their" expression, giving $1.201\ldots\text{ m}$ | Evaluate your expression to obtain the pendulum length in metres. |
...
| 166 | d9c1f8 | CK3 | Identifies $1.15\text{ m}$ as the limiting height of the case | Use $1.15\text{ m}$ as the maximum height available inside the case. |
| 167 | d9c1f8 | R2 | Compares "their" pendulum length with $1.15\text{ m}$ | Compare your pendulum length with $1.15\text{ m}$ to decide whether it is within the case height. |
...
| 171 | d9c205 | R2 | Identifies 13 selected players who scored at least 2 goals and uses the scale factor $80\div20$ | Count the selected players who scored at least 2 goals, then multiply this count by the scale factor \(80\div20\). |
...
| 175 | d9c20d | R1 | Reads $g(\text{their }7)=3$ correctly from the graph | Use the input from your part (a) result and read the corresponding output accurately from the graph of g. |
...
| 179 | d9c20d | R2 | States $g:x\to \frac{x-1}{2}$ | Write your rearranged inverse rule in mapping notation for g. |
...
| 192 | d9c215 | R3 | Solves to obtain $n=\frac{49}{3}$ | Solve the equation for \(n\) and express the result exactly as a fraction. |
...
| 196 | d9c21d | AK2 | Substitutes correctly into $PQ^2=10^2+14^2-2(10)(14)\cos(\text{their }55°)$. | Substitute the side lengths and your angle at $H$ into $PQ^2=10^2+14^2-2(10)(14) cos(	ext{your }55°)$. |
| 197 | d9c21d | AK3 | Processes to obtain $PQ\approx11.6369$ km. | Evaluate your cosine-rule expression and take the positive square root to obtain $PQ$ in kilometres. |
...
| 200 | d9c21d | AK4 | Calculates angle $HPQ\approx80.3°$ using "their" value of $PQ$. | Use the cosine rule with the two sides from $P$ and your value of $PQ$ to calculate $ angle HPQ$. |
