...
The final repair changes only `parts` and the derived `final_answer`. Its preview checks the prior canonical answer and alternatives, validates every question before writes, uses `updated_at` as a concurrent-edit guard, and is repeatable. It does not read or write historical attempts.
