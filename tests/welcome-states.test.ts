...
// say — the student's address in full on every state, no claim that we
...
describe('welcome, four states', () => {
  ...
  it('C · not yet registered: the address in full, the create door locked to it', () => {
    expect(text.unregistered).toBe(
      '✓ Payment received . The access is waiting on kiara@example.com . Create the account on that address and it is applied. Create the account Nothing expires while you do this. The payment stays attached to that address. ' + HELP,
    );
  ...
  it('D · bought for someone else: the address in full, no claim of an email sent, no pronouns for the student', () => {
    expect(text.other).toBe(
      'Sign out ✓ Thank you . Access is on kiara@example.com , running to May/June 2027 . Whoever sits the exam creates their account with that address, or signs in if they have one. Create an account ON THE ADDRESS THE ACCESS IS ON Sign in ' +
