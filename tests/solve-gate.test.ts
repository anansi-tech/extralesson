...
/**
 * THE READ-OFF CHECK LEFT THIS FILE. It used to ask the solver whether a part's
 * answer was "readable off the figure" — a model flag, which across three runs
 * of the whole bank agreed with itself on 42 of 63 slots. It is now a rule
 * about the rubric, in lib/generation/read-off.ts, and covered by
 * tests/read-off-rubric.test.ts. rubricMarksFor went with it; nothing else read
 * the solver's read_off_figure.
 */
