...
describe('(1) a form row depends on the value or a method row on its slot', () => {
  ...
  it('runs on the marking path and in the eval', () => {
    expect(at("app", "study", "session", "[id]", "mark-working.ts")).toMatch(/applyFormatDependency\(\s*requireGrounding\(requireEvidence\(oneDecisionPerRow\(result\.decisions[\s\S]*question\.rubric \?\? \[\],\s*settled,?\s*\)/);
